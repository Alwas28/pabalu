<x-app-layout>
<x-slot name="pageTitle">{{ $province->name }} — Kabupaten / Kota</x-slot>
...
.badge-kabupaten{background:rgba(16,185,129,.12);color:#34d399}
.badge-kota{background:rgba(251,191,36,.12);color:#fbbf24}
...
.modal-box{background:var(--surface);border:1px solid var(--border);border-radius:18px;width:100%;max-width:440px;overflow:hidden}
...
<div style="max-width:900px;margin:0 auto">
  ...
  <div style="display:flex;align-items:center;gap:8px;margin-bottom:22px;font-size:13px;color:var(--muted);flex-wrap:wrap">
    ...
    <i class="fa-solid fa-chevron-right" style="font-size:10px"></i>
    <span style="color:var(--text);font-weight:500">{{ $province->name }}</span>
  </div>
  ...
  <div class="card" style="margin-bottom:16px">
    <div style="padding:18px 22px;display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap">
      <div style="display:flex;align-items:center;gap:12px">
        <a href="{{ route('wilayah.index') }}" style="width:34px;height:34px;border-radius:9px;background:var(--surface2);border:1px solid var(--border);color:var(--sub);display:grid;place-items:center;font-size:13px;text-decoration:none;flex-shrink:0"
          onmouseover="this.style.color='var(--text)'" onmouseout="this.style.color='var(--sub)'">
          <i class="fa-solid fa-arrow-left"></i>
        </a>
        <div style="width:40px;height:40px;border-radius:11px;background:rgba(251,191,36,.14);color:#fbbf24;display:grid;place-items:center;font-size:17px;flex-shrink:0">
          <i class="fa-solid fa-city"></i>
        </div>
        <div>
          <div style="font-size:16px;font-weight:700;color:var(--text)">{{ $province->name }}</div>
          <div style="font-size:12px;color:var(--muted)">{{ $regencies->count() }} kabupaten/kota</div>
        </div>
      </div>
      <button class="btn-primary" onclick="openModal('modal-add')">
        <i class="fa-solid fa-plus"></i> Tambah Kab/Kota
      </button>
    </div>
  </div>
      ...
      <table class="tbl">
        <thead>
          <tr>
            ...
            <th>Nama</th>
            <th style="width:140px;text-align:center">Kecamatan</th>
            <th style="width:210px;text-align:right;padding-right:18px">Aksi</th>
          </tr>
        </thead>
        <tbody>
          @forelse($regencies as $i => $reg)
          <tr>
            <td style="color:var(--muted);font-size:12px">{{ $i + 1 }}</td>
            <td>
              <span class="badge {{ $reg->type === 'kota' ? 'badge-kota' : 'badge-kabupaten' }}">
                {{ ucfirst($reg->type) }}
              </span>
            </td>
            <td>
              <div style="font-weight:600;color:var(--text)">{{ $reg->name }}</div>
            </td>
            <td style="text-align:center">
              <span class="badge badge-blue">{{ $reg->districts_count }}</span>
            </td>
            <td style="text-align:right;padding-right:14px">
              <div style="display:flex;justify-content:flex-end;gap:6px;flex-wrap:wrap">
                <a href="{{ route('wilayah.provinces.regencies.show', [$province, $reg]) }}" class="btn-icon">
                  <i class="fa-solid fa-eye"></i> Kecamatan
                </a>
                <button class="btn-edit" onclick="openEdit({{ $reg->id }}, '{{ addslashes($reg->name) }}', '{{ $reg->type }}')">
                  <i class="fa-solid fa-pen"></i> Edit
                </button>
                <form method="POST" action="{{ route('wilayah.provinces.regencies.destroy', [$province, $reg]) }}"
                  onsubmit="return confirm('Hapus {{ addslashes($reg->full_name) }}? Semua kecamatannya akan terhapus.')">
                  @csrf @method('DELETE')
          ...
          <tr>
            <td colspan="5" style="text-align:center;padding:32px;color:var(--muted)">
              <i class="fa-solid fa-city" style="font-size:24px;margin-bottom:8px;display:block;opacity:.4"></i>
              Belum ada kabupaten/kota. Klik "Tambah Kab/Kota" untuk memulai.
            </td>
          </tr>
          @endforelse
        </tbody>
...
<div class="modal-overlay" id="modal-add" onclick="closeIfOverlay(event,'modal-add')">
  <div class="modal-box">
    <div class="modal-header">
      <div style="font-size:15px;font-weight:700;color:var(--text)">Tambah Kabupaten / Kota</div>
    ...
    <form method="POST" action="{{ route('wilayah.provinces.regencies.store', $province) }}">
      @csrf
      <div class="modal-body">
        <div>
          <label class="f-label">Jenis <span style="color:#f87171">*</span></label>
          <select name="type" class="f-input">
            <option value="kabupaten" {{ old('type') !== 'kota' ? 'selected' : '' }}>Kabupaten</option>
            <option value="kota" {{ old('type') === 'kota' ? 'selected' : '' }}>Kota</option>
          </select>
        </div>
        <div>
          <label class="f-label">Nama <span style="color:#f87171">*</span></label>
          <input type="text" name="name" class="f-input" placeholder="Contoh: Kendari" value="{{ old('name') }}" autofocus>
          @error('name')
...
<div class="modal-overlay" id="modal-edit" onclick="closeIfOverlay(event,'modal-edit')">
  <div class="modal-box">
    <div class="modal-header">
      <div style="font-size:15px;font-weight:700;color:var(--text)">Edit Kabupaten / Kota</div>
    ...
    <form method="POST" id="edit-form" action="">
      ...
      <div class="modal-body">
        <div>
          <label class="f-label">Jenis <span style="color:#f87171">*</span></label>
          <select name="type" id="edit-type" class="f-input">
        ...
        <div>
          <label class="f-label">Nama <span style="color:#f87171">*</span></label>
          <input type="text" name="name" id="edit-name" class="f-input" placeholder="Nama kabupaten/kota">
        </div>
      </div>
      <div class="modal-footer">
...
@push('scripts')
<script>
const baseUrl = '{{ url("wilayah/provinces/" . $province->id . "/regencies") }}';
