...
@php
  ...
  $payLabels  = ['cash'=>'Tunai','qris'=>'QRIS','qris_transfer'=>'QRIS Transfer','qris_pay'=>'QRIS Pay','transfer'=>'Transfer Bank','card'=>'Kartu Debit/Kredit'];
  ...
  $outCode    = $outlet->code ?? '????';
  $isMerged   = $transaction->orders->count() > 1;
@endphp
...
{{-- ══ ITEM BELANJA ══ --}}
@if($isMerged)
<div class="small bold" style="margin-bottom:3px">Struk gabungan ({{ $transaction->orders->count() }} order)</div>
@foreach($transaction->orders as $o)
<div class="small bold" style="margin:4px 0 2px;text-transform:uppercase">
  {{ $o->order_number }} — {{ $o->customer_name ?: ($o->table_number ? 'Meja '.$o->table_number : 'Walk-in') }}
</div>
@foreach($transaction->items->where('order_id', $o->id) as $item)
<div class="item-row">
...
<div class="small" style="display:flex;justify-content:space-between;margin-top:2px">
  <span class="dimmed">No. Referensi</span>
  <span>{{ $transaction->reference_number }}</span>
</div>
@endif
@if($transaction->proof_image)
<div class="small dimmed" style="margin-top:2px">Bukti transaksi terlampir (lihat di sistem)</div>
@endif
