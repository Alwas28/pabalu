@props(['outlet', 'pageTitle' => 'Dashboard Rental'])
...
#outlet-ctx{display:flex;align-items:center;gap:12px;padding:14px 16px;margin-bottom:2px;background:var(--ac-lt2);border-bottom:1px solid rgba(var(--ac-rgb),.15);flex-shrink:0}
#outlet-ctx .oc-icon{width:36px;height:36px;border-radius:10px;background:var(--ac-lt);color:var(--ac);display:grid;place-items:center;font-size:14px;flex-shrink:0}
...
.modal-box{background:var(--surface);border:1px solid var(--border);border-radius:20px;width:100%;max-width:440px;max-height:calc(100vh - 32px);overflow-y:auto;box-shadow:0 24px 64px rgba(0,0,0,.5);transform:translateY(16px);transition:transform .25s}
...
<aside id="sb">
  ...
  <div id="outlet-ctx">
    <div class="oc-icon">
      <i class="fa-solid {{ $type?->icon ?? 'fa-key' }}"></i>
    </div>
  ...
  @if($isKasir)
    ...
    <a href="{{ route('dashboard') }}" class="nav-item nav-inactive">
    ...
    <a href="{{ $outlet->route('customers.index') }}" class="nav-item {{ $outlet->routeIs('customers.*') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-users" style="width:15px;text-align:center;font-size:13px;color:#a78bfa"></i>Pelanggan
    </a>
    @if($outlet->enable_booking)
    <a href="{{ $outlet->route('bookings.create') }}" class="nav-item {{ $outlet->routeIs('bookings.*') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-calendar-plus" style="width:15px;text-align:center;font-size:13px;color:#60a5fa"></i>Booking
    </a>
    @endif
    <a href="{{ $outlet->route('rentals.active') }}" class="nav-item {{ $outlet->routeIs('rentals.*') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-key" style="width:15px;text-align:center;font-size:13px;color:#fbbf24"></i>Sewa
    </a>
    <a href="{{ $outlet->route('availability.index') }}" class="nav-item {{ $outlet->routeIs('availability.*') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-box" style="width:15px;text-align:center;font-size:13px;color:#34d399"></i>Barang &amp; Ketersediaan
    </a>
    ...
    <a href="{{ route('dashboard') }}?outlet={{ $ao->id }}" class="nav-item nav-inactive">
      <i class="fa-solid fa-store" style="width:15px;text-align:center;font-size:13px"></i>{{ \Illuminate\Support\Str::limit($ao->name, 20) }}
    </a>
    @endforeach
    @endif

    <p class="nav-section">Pengaturan</p>
    <a href="{{ route('profile.edit') }}" class="nav-item {{ request()->routeIs('profile.*') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-user-gear" style="width:15px;text-align:center;font-size:13px"></i>Akun Saya
    </a>

  @else
    {{-- ══════════════════════════════════════════════════════════ --}}
    {{-- ADMIN / OWNER / ADMIN_OUTLET NAV (RENTAL)                   --}}
    {{-- ══════════════════════════════════════════════════════════ --}}

    <p class="nav-section" style="margin-top:8px">Utama</p>
    <a href="{{ $outlet->route('show') }}" class="nav-item {{ $outlet->routeIs('show') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-store" style="width:15px;text-align:center;font-size:13px"></i>Dashboard Toko
    </a>

    @if($outlet->enable_booking)
    <p class="nav-section">Booking</p>
    <a href="{{ $outlet->route('bookings.create') }}" class="nav-item {{ $outlet->routeIs('bookings.create') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-calendar-plus" style="width:15px;text-align:center;font-size:13px;color:#60a5fa"></i>Booking Baru
    </a>
    <a href="{{ $outlet->route('bookings.active') }}" class="nav-item {{ $outlet->routeIs('bookings.active') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-calendar-check" style="width:15px;text-align:center;font-size:13px"></i>Booking Aktif
    </a>
    <a href="{{ $outlet->route('calendar.index') }}" class="nav-item {{ $outlet->routeIs('calendar.index') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-calendar-days" style="width:15px;text-align:center;font-size:13px"></i>Kalender Rental
    </a>
    <a href="{{ $outlet->route('bookings.done') }}" class="nav-item {{ $outlet->routeIs('bookings.done') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-calendar-xmark" style="width:15px;text-align:center;font-size:13px"></i>Booking Selesai/Batal
    </a>
    @endif

    <p class="nav-section">Sewa</p>
    <a href="{{ $outlet->route('rentals.create') }}" class="nav-item {{ $outlet->routeIs('rentals.create') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-key" style="width:15px;text-align:center;font-size:13px;color:#fbbf24"></i>Sewa Baru
    </a>
    <a href="{{ $outlet->route('rentals.active') }}" class="nav-item {{ $outlet->routeIs('rentals.active') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-hourglass-half" style="width:15px;text-align:center;font-size:13px"></i>Sewa Aktif
    </a>
    <a href="{{ $outlet->route('rentals.extend') }}" class="nav-item {{ $outlet->routeIs('rentals.extend') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-rotate" style="width:15px;text-align:center;font-size:13px"></i>Perpanjangan
    </a>
    <a href="{{ $outlet->route('rentals.returns') }}" class="nav-item {{ $outlet->routeIs('rentals.returns') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-right-left" style="width:15px;text-align:center;font-size:13px"></i>Pengembalian
    </a>

    <p class="nav-section">Barang</p>
    <a href="{{ $outlet->route('items.index') }}" class="nav-item {{ $outlet->routeIs('items.index') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-box" style="width:15px;text-align:center;font-size:13px;color:#34d399"></i>Barang Sewa
    </a>
    <a href="{{ $outlet->route('units.index') }}" class="nav-item {{ $outlet->routeIs('units.index') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-cubes" style="width:15px;text-align:center;font-size:13px"></i>Unit
    </a>
    <a href="{{ $outlet->route('availability.index') }}" class="nav-item {{ $outlet->routeIs('availability.index') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-calendar-week" style="width:15px;text-align:center;font-size:13px"></i>Ketersediaan
    </a>
    <a href="{{ $outlet->route('maintenance.index') }}" class="nav-item {{ $outlet->routeIs('maintenance.index') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-screwdriver-wrench" style="width:15px;text-align:center;font-size:13px"></i>Maintenance
    </a>

    <p class="nav-section">Transaksi</p>
    <a href="{{ $outlet->route('payments.index') }}" class="nav-item {{ $outlet->routeIs('payments.index') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-money-bill-wave" style="width:15px;text-align:center;font-size:13px"></i>Pembayaran
    </a>
    <a href="{{ $outlet->route('deposits.index') }}" class="nav-item {{ $outlet->routeIs('deposits.index') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-piggy-bank" style="width:15px;text-align:center;font-size:13px"></i>Deposit
    </a>
    <a href="{{ $outlet->route('fines.index') }}" class="nav-item {{ $outlet->routeIs('fines.index') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-triangle-exclamation" style="width:15px;text-align:center;font-size:13px"></i>Denda
    </a>
    <a href="{{ $outlet->route('refunds.index') }}" class="nav-item {{ $outlet->routeIs('refunds.index') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-rotate-left" style="width:15px;text-align:center;font-size:13px"></i>Refund
    </a>
    @if($p('expense.view'))
    <a href="{{ $outlet->route('expenses.index') }}" class="nav-item {{ $outlet->routeIs('expenses.*') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-file-invoice-dollar" style="width:15px;text-align:center;font-size:13px"></i>Pengeluaran
    </a>
    @endif

    <p class="nav-section">Pelanggan</p>
    <a href="{{ $outlet->route('customers.index') }}" class="nav-item {{ $outlet->routeIs('customers.index') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-users" style="width:15px;text-align:center;font-size:13px;color:#a78bfa"></i>Semua Pelanggan
    </a>
    <a href="{{ $outlet->route('customers.create') }}" class="nav-item {{ $outlet->routeIs('customers.create') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-user-plus" style="width:15px;text-align:center;font-size:13px"></i>Tambah Pelanggan
    </a>
    <a href="{{ $outlet->route('documents.index') }}" class="nav-item {{ $outlet->routeIs('documents.index') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-id-card" style="width:15px;text-align:center;font-size:13px"></i>Dokumen
    </a>

    <p class="nav-section">Laporan</p>
    <a href="{{ $outlet->route('reports.revenue') }}" class="nav-item {{ $outlet->routeIs('reports.revenue') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-chart-line" style="width:15px;text-align:center;font-size:13px"></i>Pendapatan
    </a>
    <a href="{{ $outlet->route('reports.rentals') }}" class="nav-item {{ $outlet->routeIs('reports.rentals') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-chart-column" style="width:15px;text-align:center;font-size:13px"></i>Rental
    </a>
    <a href="{{ $outlet->route('reports.items') }}" class="nav-item {{ $outlet->routeIs('reports.items') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-boxes-stacked" style="width:15px;text-align:center;font-size:13px"></i>Barang
    </a>
    <a href="{{ $outlet->route('reports.fines') }}" class="nav-item {{ $outlet->routeIs('reports.fines') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-chart-pie" style="width:15px;text-align:center;font-size:13px"></i>Denda
    </a>
    <a href="{{ $outlet->route('reports.deposits') }}" class="nav-item {{ $outlet->routeIs('reports.deposits') ? 'nav-active' : 'nav-inactive' }}">
      <i class="fa-solid fa-vault" style="width:15px;text-align:center;font-size:13px"></i>Deposit
    </a>
    @if($p('report.profit_loss'))
    <a href="{{ $outlet->route('reports.profit-loss') }}" class="nav-item {{ $outlet->routeIs('reports.profit-loss*') ? 'nav-active' : 'nav-inactive' }}">
    ...
    <div style="display:flex;align-items:center;gap:14px">
      ...
      <div>
        <div style="font-size:11px;color:var(--muted);margin-bottom:1px;display:flex;align-items:center;gap:6px">
          <i class="fa-solid {{ $type?->icon ?? 'fa-key' }}" style="color:var(--ac);font-size:10px"></i>
          {{ $outlet->name }}
        </div>
