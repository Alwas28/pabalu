<x-public-layout :title="$homeCategory->name . ' — Pabalu'" :description="$homeCategory->description">

@push('styles')
  .hcat-hero{background:var(--mint-50);padding:32px 0;display:flex;align-items:center;gap:16px;}
  .hcat-hero .icon{width:56px;height:56px;border-radius:14px;background:#fff;border:1px solid var(--line);display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0;}
  .hcat-hero .icon img{width:100%;height:100%;object-fit:cover;}
  .hcat-hero .icon span{font-size:22px;font-weight:800;color:var(--green-700);}
...
<section class="hcat-hero">
  <div class="container" style="display:flex;align-items:center;gap:16px">
    <div class="icon">
      ...
      <img src="{{ asset('storage/' . $homeCategory->image) }}" alt="{{ $homeCategory->name }}">
      @else
      <span>{{ mb_substr($homeCategory->name, 0, 1) }}</span>
      @endif
    </div>
    <div>
      <h1>{{ $homeCategory->name }}</h1>
      @if($homeCategory->description)
      <p>{{ $homeCategory->description }}</p>
      @endif
    </div>
  </div>
</section>
