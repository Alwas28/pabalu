<x-outlet-layout :outlet="$outlet" pageTitle="Pendapatan">
...
@php
  $paymentMeta = [
    ...
    'deposit'       => ['label'=>'Potongan Deposit', 'icon'=>'fa-piggy-bank',       'color'=>'#f472b6'],
  ];
...
<div class="animate-fadeUp">
  <h2 class="font-display" style="font-size:20px;font-weight:700;color:var(--text)">Pendapatan</h2>
...
<div class="card animate-fadeUp d1" style="padding:16px 20px">
  <form method="GET" action="{{ $outlet->route('reports.revenue') }}"
    ...
    <div>
      ...
      <select name="payment_method" class="f-input" style="width:160px">
    ...
    <div style="display:flex;gap:6px;margin-left:auto">
      ...
      <a href="{{ $outlet->route('reports.revenue', ['from' => $f, 'to' => $t]) }}"
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px" class="animate-fadeUp d2">
  ...
  <div class="stat-card">
    ...
    <div>
      <div class="stat-num" style="font-size:17px">Rp {{ number_format($avgPayment) }}</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px" class="animate-fadeUp d3">
  ...
  <div class="card">
    <div class="card-header">
      ...
      <span style="font-size:12px;color:var(--muted)">{{ $byItem->count() }} barang</span>
    </div>
    <div style="overflow-x:auto">
      <table class="tbl">
        <thead>
          <tr>
            ...
            <th>Barang</th>
            <th style="text-align:center;width:70px">Trx</th>
            <th style="text-align:right;width:150px">Pendapatan</th>
            <th style="text-align:right;width:55px">%</th>
          </tr>
        </thead>
        <tbody>
          @forelse($byItem as $i => $item)
          @php $pct = $totalRevenue > 0 ? round($item->total_revenue / $totalRevenue * 100, 1) : 0; @endphp
          <tr>
            <td style="font-size:11px;font-weight:700;color:{{ $i < 3 ? 'var(--ac)' : 'var(--muted)' }}">
              {{ $i + 1 }}
            </td>
            <td class="td-main">{{ $item->item_name }}</td>
            <td style="text-align:center;font-weight:700;color:var(--text)">{{ number_format($item->total_trx) }}</td>
            <td style="text-align:right;font-weight:700;color:var(--text)">Rp {{ number_format($item->total_revenue) }}</td>
...
<div class="card animate-fadeUp d3">
  <div class="card-header">
    ...
    <span style="font-size:12px;color:var(--muted)">{{ number_format($totalCount) }} pembayaran</span>
  </div>
    ...
    <table class="tbl">
      <thead>
        <tr>
          ...
          <th>Pelanggan / Barang</th>
          <th style="text-align:right;width:130px">Jumlah</th>
          <th style="width:130px">Metode</th>
        </tr>
      </thead>
      <tbody>
        @foreach($payments->take(200) as $p)
        @php $pm = $paymentMeta[$p->method] ?? ['label'=>$p->method,'color'=>'#94a3b8']; @endphp
        <tr>
          <td>
            <div style="font-weight:600;font-size:13px;color:var(--text)">{{ $p->paid_at->translatedFormat('d M Y') }}</div>
            <div style="font-size:11px;color:var(--muted)">{{ $p->paid_at->format('H:i') }}</div>
          </td>
          <td style="font-size:12px;color:var(--sub);font-family:monospace">{{ $p->rentalTransaction->order_number }}</td>
          <td style="font-size:12.5px;color:var(--sub)">
            {{ $p->rentalTransaction->customer->name }} —
            {{ $p->rentalTransaction->rentalUnit->rentalItem->name }}
            @if($p->is_fine)
            <span class="badge badge-red" style="margin-left:4px">Denda</span>
            @endif
          </td>
          <td style="text-align:right;font-weight:700;color:var(--text)">Rp {{ number_format($p->amount) }}</td>
          <td>
  ...
  <div style="padding:12px 20px;border-top:1px solid var(--border);font-size:12.5px;color:var(--muted);text-align:center">
    Menampilkan 200 dari {{ number_format($payments->count()) }} pembayaran. Persempit rentang tanggal untuk melihat lebih detail.
  </div>
  @endif
  @endif
</div>
