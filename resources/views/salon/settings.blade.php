...
<div class="card animate-fadeUp d1" style="margin-bottom:20px">
  ...
  <div class="card-body" style="display:flex;flex-direction:column;gap:0">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;padding:20px 0 16px;border-top:1px solid var(--border)">
      <div style="flex:1;padding-right:24px">
        <div style="font-size:14px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:8px">
          <i class="fa-solid fa-qrcode" style="color:var(--ac);font-size:13px"></i>
          Self Order (Pesan Mandiri)
        </div>
        <div style="font-size:12.5px;color:var(--muted);margin-top:4px;line-height:1.6">
          Izinkan pelanggan memesan secara mandiri melalui QR code menu publik.
          Setiap pesanan yang masuk akan ditinjau oleh staf sebelum diproses.
        </div>
        <div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap">
          <span style="font-size:11px;padding:2px 8px;border-radius:99px;background:var(--ac-lt);color:var(--ac);font-weight:600">
            <i class="fa-solid fa-check" style="margin-right:3px"></i>Scan QR &amp; pesan
          </span>
          <span style="font-size:11px;padding:2px 8px;border-radius:99px;background:var(--ac-lt);color:var(--ac);font-weight:600">
            <i class="fa-solid fa-check" style="margin-right:3px"></i>Tinjau sebelum diproses
          </span>
        </div>
      </div>
      <label style="position:relative;display:inline-block;width:48px;height:26px;flex-shrink:0;cursor:pointer">
        <input type="checkbox" name="enable_self_order" value="1" id="toggle-self-order"
          ...
          style="opacity:0;width:0;height:0;position:absolute"
          onchange="document.getElementById('self-order-status').textContent = this.checked ? 'Aktif' : 'Nonaktif';
                    document.getElementById('self-order-status').style.color = this.checked ? 'var(--ac)' : 'var(--muted)'">
  ...
  <div class="card-body" style="display:flex;flex-direction:column;gap:0">
    ...
    @php
      $pmList = [
        ...
        ['enable_qris_pay',      'qris-pay',      'QRIS Pay (Midtrans)','fa-bolt',             'Sistem membuat QRIS dinamis via Midtrans. Pembayaran terverifikasi otomatis.'],
        ['enable_transfer',      'transfer',      'Transfer Bank',      'fa-building-columns', 'Transfer antar bank / virtual account.'],
        ['enable_card',          'card',          'Kartu (EDC)',        'fa-credit-card',      'Pembayaran via mesin EDC, kasir input nomor referensi & upload foto struk.'],
      ];
    @endphp
