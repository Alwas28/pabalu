...
@media (max-width: 768px) {
  .form-grid-outlet { grid-template-columns: 1fr; }
}
</style>
@endpush
  ...
  <form method="POST" action="{{ route('outlets.update', $outlet) }}">
    ...
    {{-- Hidden field agar is_active = 0 terkirim saat checkbox tidak dicentang --}}
    <input type="hidden" name="is_active" value="0">

    <div class="form-grid-outlet">
              ...
              <div style="position:relative;flex-shrink:0">
                ...
                <div id="toggle-thumb"
                  style="position:absolute;top:3px;left:3px;width:16px;height:16px;border-radius:50%;
                         background:#fff;pointer-events:none;transition:transform .2s"></div>
              </div>
            </label>

            <div id="inactive-warning"
              style="margin-top:12px;padding:10px 12px;border-radius:9px;
                     background:rgba(248,113,113,.08);border:1px solid rgba(248,113,113,.2);
                     font-size:12px;color:#f87171;line-height:1.6;display:none">
              <i class="fa-solid fa-triangle-exclamation" style="margin-right:5px"></i>
              Outlet nonaktif tidak bisa diakses kasir, produk tidak muncul di POS, dan halaman order online tidak dapat dibuka.
            </div>
          </div>
        </div>
  ...
  var warning = document.getElementById('inactive-warning');
