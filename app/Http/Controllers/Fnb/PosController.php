...
class PosController extends Controller
{
    ...
    public function index(Outlet $outlet): View|RedirectResponse
    {
        ...
        return view('fnb.pos.index', compact(
    ...
    public function store(Request $request, Outlet $outlet): JsonResponse
    {
        ...
        if ($requiresOpening) {
            ...
            if (!$this->isBusinessDayOpen($outlet)) {
                return response()->json([
                    'message' => 'Opening stok belum dilakukan. Lakukan opening stok terlebih dahulu sebelum berjualan.',
                ], 422);
            }
        }
        ...
        DB::transaction(function () use ($request, $outlet, $user, $businessDate, $requiresOpening, &$receipt, &$transaction) {
            $subtotal  = 0;
            $validated = [];
            ...
            foreach ($request->items as $item) {
                ...
                if ($requiresOpening && $product->stock < $item['qty']) {
